...
Partial Friend Class cached_ikeyvalue
    Private Class caches
        ...
        Public Function read_get(ByVal key() As Byte, ByRef value() As Byte) As Boolean
            If v.get(make_array_pointer(key), value) Then
                Return True
        ...
        Public Function sizeof_get(ByVal key() As Byte, ByRef result As Int64) As Boolean
            ...
            If s.get(make_array_pointer(key), result) Then
                Return True
            ElseIf v.get(make_array_pointer(key), b) Then
        ...
        Public Function havenot(ByVal key() As Byte) As Boolean
            ...
            Dim v3 As Int64 = 0
            Return (v.get(make_array_pointer(key), v1) AndAlso
                    v1 Is Nothing) OrElse
                   (h.get(make_array_pointer(key), v2) AndAlso
                    Not v2) OrElse
                   (s.get(make_array_pointer(key), v3) AndAlso
                    v3 = npos)
        End Function
        ...
        Public Sub read_set(ByVal key() As Byte, ByVal result() As Byte)
            If result Is Nothing OrElse array_size(result) < max_value_size Then
                v.set(make_array_pointer(key), result)
            End If
            h.set(make_array_pointer(key), Not result Is Nothing)
            s.set(make_array_pointer(key), If(result Is Nothing, npos, array_size(result)))
        End Sub

        Public Sub append_set(ByVal key() As Byte, ByVal value() As Byte, ByVal result As Boolean)
            If result Then
                h.set(make_array_pointer(key), True)
                s.erase(make_array_pointer(key))
                v.erase(make_array_pointer(key))
            End If
        End Sub

        Public Sub delete_set(ByVal key() As Byte, ByVal result As Boolean)
            h.set(make_array_pointer(key), False)
            s.set(make_array_pointer(key), npos)
            v.set(make_array_pointer(key), Nothing)
        End Sub

        Public Sub seek_set(ByVal key() As Byte, ByVal result As Boolean)
            h.set(make_array_pointer(key), result)
            If Not result Then
                v.set(make_array_pointer(key), Nothing)
                s.set(make_array_pointer(key), npos)
            End If
        ...
        Public Sub sizeof_set(ByVal key() As Byte, ByVal result As Int64)
            h.set(make_array_pointer(key), result >= 0)
            s.set(make_array_pointer(key), result)
            If result = npos Then
                v.set(make_array_pointer(key), Nothing)
            End If
        End Sub

' This file is created by genall with
' C:\Users\hzj_jie\git\osi.net\service\resource\gen\gen.exe
' _bstyle_test_data "[call_struct_on_heap]" "call_struct_on_heap.txt" "[case1]" "case1.txt" "[case2]" "case2.txt" "[delegate]" "delegate.txt" "[empty_struct_overloads]" "empty_struct_overloads.txt" "[errors_dot_is_disallowed_as_the_end_of_name]" "errors\dot-is-disallowed-as-the-end-of-name.txt" "[for_loop]" "for_loop.txt" "[func_name_with_dot]" "func_name_with_dot.txt" "[function_struct]" "function_struct.txt" "[global_variable]" "global_variable.txt" "[nested_struct]" "nested_struct.txt" "[overload_function]" "overload_function.txt" "[predefined_def]" "predefined-def.txt" "[real__file__]" "real__file__.txt" "[return_struct]" "return_struct.txt" "[single_level_struct]" "single_level_struct.txt" "[statement]" "statement.txt"

Option Explicit On
Option Infer Off
Option Strict On

'this file is generated by /osi/service/resource/gen/gen.exe with
'[call_struct_on_heap] [case1] [case2] [delegate] [empty_struct_overloads] [errors_dot_is_disallowed_as_the_end_of_name] [for_loop] [func_name_with_dot] [function_struct] [global_variable] [nested_struct] [overload_function] [predefined_def] [real__file__] [return_struct] [single_level_struct] [statement]
'so change /osi/service/resource/gen/gen.cs or resource files instead of this file

Imports osi.root.connector

Friend Module _bstyle_test_data
    Public ReadOnly [call_struct_on_heap]() As Byte
    Public ReadOnly [case1]() As Byte
    Public ReadOnly [case2]() As Byte
    Public ReadOnly [delegate]() As Byte
    Public ReadOnly [empty_struct_overloads]() As Byte
    Public ReadOnly [errors_dot_is_disallowed_as_the_end_of_name]() As Byte
    Public ReadOnly [for_loop]() As Byte
    Public ReadOnly [func_name_with_dot]() As Byte
    Public ReadOnly [function_struct]() As Byte
    Public ReadOnly [global_variable]() As Byte
    Public ReadOnly [nested_struct]() As Byte
    Public ReadOnly [overload_function]() As Byte
    Public ReadOnly [predefined_def]() As Byte
    Public ReadOnly [real__file__]() As Byte
    Public ReadOnly [return_struct]() As Byte
    Public ReadOnly [single_level_struct]() As Byte
    Public ReadOnly [statement]() As Byte

    Sub New()
        [call_struct_on_heap] = Convert.FromBase64String(strcat_hint(CUInt(432), _
        ""))
        [case1] = Convert.FromBase64String(strcat_hint(CUInt(188), _
        "77u/CiNpbmNsdWRlIDxic3R5bGUuaD4KCnZvaWQgcHJpbnQoc3RyaW5nIGkpIHsKICBzdHJpbmcgdGVtcDsKICBsb2dpYyAiaW50ZXJydXB0IHN0ZG91dCBpIHRlbXAiOwp9Cgp2b2lkIG1haW4oKSB7CiAgcHJpbnQoIkhlbGxvIFdvcmxkIik7Cn0K"))
        [case2] = Convert.FromBase64String(strcat_hint(CUInt(1348), _
        ""))
        [delegate] = Convert.FromBase64String(strcat_hint(CUInt(248), _
        "77u/CiNpbmNsdWRlIDxic3R5bGUuaD4KI2luY2x1ZGUgPHN0ZGlvLmg+CgpkZWxlZ2F0ZSB2b2lkIGZ1bmMoKTsKCnZvaWQgZigpIHsKICBwdXRjaGFyKDEwMik7Cn0KCnZvaWQgZygpIHsKICBwdXRjaGFyKDEwMyk7Cn0KCnZvaWQgbWFpbigpIHsKICBmdW5jIHg7CiAgeCA9IGY7CiAgeCgpOwogIHggPSBnOwogIHgoKTsKfQo="))
        [empty_struct_overloads] = Convert.FromBase64String(strcat_hint(CUInt(1036), _
        ""))
        [errors_dot_is_disallowed_as_the_end_of_name] = Convert.FromBase64String(strcat_hint(CUInt(44), _
        "77u/DQp2b2lkIG1haW4oKSB7DQogIGludCB4LjsNCn0="))
        [for_loop] = Convert.FromBase64String(strcat_hint(CUInt(532), _
        ""))
        [func_name_with_dot] = Convert.FromBase64String(strcat_hint(CUInt(152), _
        "77u/CiNpbmNsdWRlIDxic3R5bGUuaD4KI2luY2x1ZGUgPHN0ZGlvLmg+Cgp2b2lkIGYuZihpbnQgeCkgewogIHB1dGNoYXIoeCk7Cn0KCnZvaWQgbWFpbigpIHsKICBmLmYoMSk7CiAgZi5mKDIpOwp9"))
        [function_struct] = Convert.FromBase64String(strcat_hint(CUInt(376), _
        ""))
        [global_variable] = Convert.FromBase64String(strcat_hint(CUInt(352), _
        ""))
        [nested_struct] = Convert.FromBase64String(strcat_hint(CUInt(376), _
        ""))
        [overload_function] = Convert.FromBase64String(strcat_hint(CUInt(1088), _
        ""))
        [predefined_def] = Convert.FromBase64String(strcat_hint(CUInt(88), _
        "77u/DQojaW5jbHVkZSA8YnN0eWxlLmg+DQoNCiNpZmRlZiBCU1RZTEUNCnZvaWQgbWFpbigpIHt9DQojZW5kaWY="))
        [real__file__] = Convert.FromBase64String(strcat_hint(CUInt(184), _
        "77u/CiNpbmNsdWRlIDxic3R5bGUuaD4KCnZvaWQgcHJpbnQoc3RyaW5nIGkpIHsKICBzdHJpbmcgdGVtcDsKICBsb2dpYyAiaW50ZXJydXB0IHN0ZG91dCBpIHRlbXAiOwp9Cgp2b2lkIG1haW4oKSB7CiAgcHJpbnQoX19GSUxFX18pOwp9Cg=="))
        [return_struct] = Convert.FromBase64String(strcat_hint(CUInt(356), _
        ""))
        [single_level_struct] = Convert.FromBase64String(strcat_hint(CUInt(292), _
        ""))
        [statement] = Convert.FromBase64String(strcat_hint(CUInt(248), _
        "77u/CiNpbmNsdWRlIDxic3R5bGUuaD4KCnZvaWQgc3RkX291dChzdHJpbmcgaSkgewogIHN0cmluZyB0bXA7CiAgbG9naWMgImludGVycnVwdCBzdGRvdXQgaSB0bXAiOwp9Cgp2b2lkIG1haW4oKSB7CiAgc3RkX291dChfX1NUQVRFTUVOVF9fKTsKICBzdHJpbmcgcyA9IF9fU1RBVEVNRU5UX187CiAgc3RkX291dChzKTsKfQo="))
    End Sub
End Module

' This file is created by genall with
' C:\Users\Hzj_jie\git\osi\service\resource\gen\gen.exe
<<<<<<< HEAD
' _b2style_test_data "[_1_to_100]" "_1_to_100.txt" "[__i]" "++i.txt" "[__i__]" "++i++.txt" "[another_1_to_100]" "another_1_to_100.txt" "[biguint]" "biguint.txt" "[bool_and_bool]" "bool_and_bool.txt" "[calculate_pi_bbp]" "calculate_pi_bbp.txt" "[case1]" "case1.txt" "[case2]" "case2.txt" "[class]" "class.txt" "[class_function_with_namespace]" "class_function_with_namespace.txt" "[class_in_namespace]" "class_in_namespace.txt" "[class_inheritance]" "class_inheritance.txt" "[class_on_heap]" "class_on_heap.txt" "[comments]" "comments.txt" "[delegate]" "delegate.txt" "[delegate_template]" "delegate_template.txt" "[delegate2]" "delegate2.txt" "[empty_struct]" "empty_struct.txt" "[errors_cycle_typedef]" "errors\cycle-typedef.txt" "[errors_dollar_in_number]" "errors\dollar-in-number.txt" "[errors_duplicate_template_type_parameters]" "errors\duplicate_template_type_parameters.txt" "[errors_function_name_ends_with_dot]" "errors\function-name-ends-with-dot.txt" "[errors_function_return_struct_type_mismatch]" "errors\function-return-struct-type-mismatch.txt" "[errors_include_needs_wraps]" "errors\include-needs-wraps.txt" "[errors_missing_ending_quota]" "errors\missing-ending-quota.txt" "[errors_reinterpret_cast_heap_with_index]" "errors\reinterpret_cast_heap_with_index.txt" "[errors_reinterpret_cast_unknown_type]" "errors\reinterpret_cast_unknown_type.txt" "[errors_reinterpret_cast_unknown_variable]" "errors\reinterpret_cast_unknown_variable.txt" "[errors_reinterpret_cast_without_type_id]" "errors\reinterpret_cast_without_type_id.txt" "[errors_three_pluses]" "errors\three-pluses.txt" "[errors_undefined_value_clause]" "errors\undefined-value-clause.txt" "[errors_value_clause_struct_type_mismatch]" "errors\value-clause-struct-type-mismatch.txt" "[errors_value_clause_struct_type_mismatch2]" "errors\value-clause-struct-type-mismatch2.txt" "[for_loop]" "for_loop.txt" "[function_ptr]" "function_ptr.txt" "[function_ref]" "function_ref.txt" "[function_with_global_namespace]" "function_with_global_namespace.txt" "[heap]" "heap.txt" "[heap_declaration]" "heap_declaration.txt" "[heap_function_ref]" "heap_function_ref.txt" "[heap_ptr_to_int64]" "heap_ptr_to_int64.txt" "[i__]" "i++.txt" "[i__2]" "i++2.txt" "[ifndef]" "ifndef.txt" "[include]" "include.txt" "[include2]" "include2.txt" "[legacy_biguint_to_str]" "legacy_biguint_to_str.txt" "[loaded_method]" "loaded_method.txt" "[lots_of_semi_colons]" "lots-of-semi-colons.txt" "[multiline_string]" "multiline_string.txt" "[namespaces]" "namespaces.txt" "[negative_int]" "negative_int.txt" "[nested_class]" "nested_class.txt" "[nested_heap_access]" "nested_heap_access.txt" "[nested_paragraph]" "nested_paragraph.txt" "[nested_template]" "nested_template.txt" "[order_of_operators]" "order_of_operators.txt" "[pi_integral_0_1]" "pi_integral_0_1.txt" "[primitive_template]" "primitive_template.txt" "[reinterpret_cast]" "reinterpret_cast.txt" "[reinterpret_cast_heap]" "reinterpret_cast_heap.txt" "[self_add]" "self_add.txt" "[shift]" "shift.txt" "[str_unescape]" "str_unescape.txt" "[struct_function_ref]" "struct_function_ref.txt" "[struct_in_namespace]" "struct_in_namespace.txt" "[template]" "template.txt" "[template_template_case1]" "template_template_case1.txt" "[template_with_different_length]" "template_with_different_length.txt" "[template_wont_be_extended_twice]" "template_wont_be_extended_twice.txt" "[two_template_type_parameters]" "two_template_type_parameters.txt" "[typedef]" "typedef.txt" "[ufloat_operators]" "ufloat_operators.txt" "[ufloat_std_out]" "ufloat_std_out.txt" "[var_delegate_template]" "var_delegate_template.txt" "[while_0_to_1]" "while_0_to_1.txt" "[while_1_to_100]" "while_1_to_100.txt"
=======
' _b2style_test_data "[_1_to_100]" "_1_to_100.txt" "[__i]" "++i.txt" "[__i__]" "++i++.txt" "[another_1_to_100]" "another_1_to_100.txt" "[biguint]" "biguint.txt" "[bool_and_bool]" "bool_and_bool.txt" "[calculate_pi_bbp]" "calculate_pi_bbp.txt" "[case1]" "case1.txt" "[case2]" "case2.txt" "[class]" "class.txt" "[class_function_with_namespace]" "class_function_with_namespace.txt" "[class_in_namespace]" "class_in_namespace.txt" "[class_inheritance]" "class_inheritance.txt" "[class_on_heap]" "class_on_heap.txt" "[comments]" "comments.txt" "[delegate]" "delegate.txt" "[delegate_template]" "delegate_template.txt" "[delegate2]" "delegate2.txt" "[empty_struct]" "empty_struct.txt" "[errors_cycle_typedef]" "errors\cycle-typedef.txt" "[errors_dollar_in_number]" "errors\dollar-in-number.txt" "[errors_duplicate_template_type_parameters]" "errors\duplicate_template_type_parameters.txt" "[errors_function_name_ends_with_dot]" "errors\function-name-ends-with-dot.txt" "[errors_function_return_struct_type_mismatch]" "errors\function-return-struct-type-mismatch.txt" "[errors_include_needs_wraps]" "errors\include-needs-wraps.txt" "[errors_missing_ending_quota]" "errors\missing-ending-quota.txt" "[errors_reinterpret_cast_heap_with_index]" "errors\reinterpret_cast_heap_with_index.txt" "[errors_reinterpret_cast_unknown_type]" "errors\reinterpret_cast_unknown_type.txt" "[errors_reinterpret_cast_unknown_variable]" "errors\reinterpret_cast_unknown_variable.txt" "[errors_reinterpret_cast_without_type_id]" "errors\reinterpret_cast_without_type_id.txt" "[errors_template_without_type_parameter]" "errors\template_without_type_parameter.txt" "[errors_three_pluses]" "errors\three-pluses.txt" "[errors_undefined_value_clause]" "errors\undefined-value-clause.txt" "[errors_value_clause_struct_type_mismatch]" "errors\value-clause-struct-type-mismatch.txt" "[errors_value_clause_struct_type_mismatch2]" "errors\value-clause-struct-type-mismatch2.txt" "[for_loop]" "for_loop.txt" "[function_ptr]" "function_ptr.txt" "[function_ref]" "function_ref.txt" "[function_with_global_namespace]" "function_with_global_namespace.txt" "[heap]" "heap.txt" "[heap_declaration]" "heap_declaration.txt" "[heap_function_ref]" "heap_function_ref.txt" "[heap_ptr_to_int64]" "heap_ptr_to_int64.txt" "[i__]" "i++.txt" "[i__2]" "i++2.txt" "[ifndef]" "ifndef.txt" "[include]" "include.txt" "[include2]" "include2.txt" "[legacy_biguint_to_str]" "legacy_biguint_to_str.txt" "[loaded_method]" "loaded_method.txt" "[lots_of_semi_colons]" "lots-of-semi-colons.txt" "[multiline_string]" "multiline_string.txt" "[namespaces]" "namespaces.txt" "[negative_int]" "negative_int.txt" "[nested_class]" "nested_class.txt" "[nested_heap_access]" "nested_heap_access.txt" "[nested_paragraph]" "nested_paragraph.txt" "[nested_template]" "nested_template.txt" "[order_of_operators]" "order_of_operators.txt" "[pi_integral_0_1]" "pi_integral_0_1.txt" "[primitive_template]" "primitive_template.txt" "[reinterpret_cast]" "reinterpret_cast.txt" "[reinterpret_cast_heap]" "reinterpret_cast_heap.txt" "[self_add]" "self_add.txt" "[shift]" "shift.txt" "[str_unescape]" "str_unescape.txt" "[struct_function_ref]" "struct_function_ref.txt" "[struct_in_namespace]" "struct_in_namespace.txt" "[template]" "template.txt" "[template_template_case1]" "template_template_case1.txt" "[template_with_different_length]" "template_with_different_length.txt" "[template_wont_be_extended_twice]" "template_wont_be_extended_twice.txt" "[two_template_type_parameters]" "two_template_type_parameters.txt" "[typedef]" "typedef.txt" "[ufloat_operators]" "ufloat_operators.txt" "[ufloat_std_out]" "ufloat_std_out.txt" "[while_0_to_1]" "while_0_to_1.txt" "[while_1_to_100]" "while_1_to_100.txt"
>>>>>>> master

Option Explicit On
Option Infer Off
Option Strict On

'this file is generated by /osi/service/resource/gen/gen.exe with
<<<<<<< HEAD
'[_1_to_100] [__i] [__i__] [another_1_to_100] [biguint] [bool_and_bool] [calculate_pi_bbp] [case1] [case2] [class] [class_function_with_namespace] [class_in_namespace] [class_inheritance] [class_on_heap] [comments] [delegate] [delegate_template] [delegate2] [empty_struct] [errors_cycle_typedef] [errors_dollar_in_number] [errors_duplicate_template_type_parameters] [errors_function_name_ends_with_dot] [errors_function_return_struct_type_mismatch] [errors_include_needs_wraps] [errors_missing_ending_quota] [errors_reinterpret_cast_heap_with_index] [errors_reinterpret_cast_unknown_type] [errors_reinterpret_cast_unknown_variable] [errors_reinterpret_cast_without_type_id] [errors_three_pluses] [errors_undefined_value_clause] [errors_value_clause_struct_type_mismatch] [errors_value_clause_struct_type_mismatch2] [for_loop] [function_ptr] [function_ref] [function_with_global_namespace] [heap] [heap_declaration] [heap_function_ref] [heap_ptr_to_int64] [i__] [i__2] [ifndef] [include] [include2] [legacy_biguint_to_str] [loaded_method] [lots_of_semi_colons] [multiline_string] [namespaces] [negative_int] [nested_class] [nested_heap_access] [nested_paragraph] [nested_template] [order_of_operators] [pi_integral_0_1] [primitive_template] [reinterpret_cast] [reinterpret_cast_heap] [self_add] [shift] [str_unescape] [struct_function_ref] [struct_in_namespace] [template] [template_template_case1] [template_with_different_length] [template_wont_be_extended_twice] [two_template_type_parameters] [typedef] [ufloat_operators] [ufloat_std_out] [var_delegate_template] [while_0_to_1] [while_1_to_100]
=======
'[_1_to_100] [__i] [__i__] [another_1_to_100] [biguint] [bool_and_bool] [calculate_pi_bbp] [case1] [case2] [class] [class_function_with_namespace] [class_in_namespace] [class_inheritance] [class_on_heap] [comments] [delegate] [delegate_template] [delegate2] [empty_struct] [errors_cycle_typedef] [errors_dollar_in_number] [errors_duplicate_template_type_parameters] [errors_function_name_ends_with_dot] [errors_function_return_struct_type_mismatch] [errors_include_needs_wraps] [errors_missing_ending_quota] [errors_reinterpret_cast_heap_with_index] [errors_reinterpret_cast_unknown_type] [errors_reinterpret_cast_unknown_variable] [errors_reinterpret_cast_without_type_id] [errors_template_without_type_parameter] [errors_three_pluses] [errors_undefined_value_clause] [errors_value_clause_struct_type_mismatch] [errors_value_clause_struct_type_mismatch2] [for_loop] [function_ptr] [function_ref] [function_with_global_namespace] [heap] [heap_declaration] [heap_function_ref] [heap_ptr_to_int64] [i__] [i__2] [ifndef] [include] [include2] [legacy_biguint_to_str] [loaded_method] [lots_of_semi_colons] [multiline_string] [namespaces] [negative_int] [nested_class] [nested_heap_access] [nested_paragraph] [nested_template] [order_of_operators] [pi_integral_0_1] [primitive_template] [reinterpret_cast] [reinterpret_cast_heap] [self_add] [shift] [str_unescape] [struct_function_ref] [struct_in_namespace] [template] [template_template_case1] [template_with_different_length] [template_wont_be_extended_twice] [two_template_type_parameters] [typedef] [ufloat_operators] [ufloat_std_out] [while_0_to_1] [while_1_to_100]
>>>>>>> master
'so change /osi/service/resource/gen/gen.cs or resource files instead of this file

Imports osi.root.connector

Friend Module _b2style_test_data
    Public ReadOnly [_1_to_100]() As Byte
    Public ReadOnly [__i]() As Byte
    Public ReadOnly [__i__]() As Byte
    Public ReadOnly [another_1_to_100]() As Byte
    Public ReadOnly [biguint]() As Byte
    Public ReadOnly [bool_and_bool]() As Byte
    Public ReadOnly [calculate_pi_bbp]() As Byte
    Public ReadOnly [case1]() As Byte
    Public ReadOnly [case2]() As Byte
    Public ReadOnly [class]() As Byte
    Public ReadOnly [class_function_with_namespace]() As Byte
    Public ReadOnly [class_in_namespace]() As Byte
    Public ReadOnly [class_inheritance]() As Byte
    Public ReadOnly [class_on_heap]() As Byte
    Public ReadOnly [comments]() As Byte
    Public ReadOnly [delegate]() As Byte
    Public ReadOnly [delegate_template]() As Byte
    Public ReadOnly [delegate2]() As Byte
    Public ReadOnly [empty_struct]() As Byte
    Public ReadOnly [errors_cycle_typedef]() As Byte
    Public ReadOnly [errors_dollar_in_number]() As Byte
    Public ReadOnly [errors_duplicate_template_type_parameters]() As Byte
    Public ReadOnly [errors_function_name_ends_with_dot]() As Byte
    Public ReadOnly [errors_function_return_struct_type_mismatch]() As Byte
    Public ReadOnly [errors_include_needs_wraps]() As Byte
    Public ReadOnly [errors_missing_ending_quota]() As Byte
    Public ReadOnly [errors_reinterpret_cast_heap_with_index]() As Byte
    Public ReadOnly [errors_reinterpret_cast_unknown_type]() As Byte
    Public ReadOnly [errors_reinterpret_cast_unknown_variable]() As Byte
    Public ReadOnly [errors_reinterpret_cast_without_type_id]() As Byte
    Public ReadOnly [errors_template_without_type_parameter]() As Byte
    Public ReadOnly [errors_three_pluses]() As Byte
    Public ReadOnly [errors_undefined_value_clause]() As Byte
    Public ReadOnly [errors_value_clause_struct_type_mismatch]() As Byte
    Public ReadOnly [errors_value_clause_struct_type_mismatch2]() As Byte
    Public ReadOnly [for_loop]() As Byte
    Public ReadOnly [function_ptr]() As Byte
    Public ReadOnly [function_ref]() As Byte
    Public ReadOnly [function_with_global_namespace]() As Byte
    Public ReadOnly [heap]() As Byte
    Public ReadOnly [heap_declaration]() As Byte
    Public ReadOnly [heap_function_ref]() As Byte
    Public ReadOnly [heap_ptr_to_int64]() As Byte
    Public ReadOnly [i__]() As Byte
    Public ReadOnly [i__2]() As Byte
    Public ReadOnly [ifndef]() As Byte
    Public ReadOnly [include]() As Byte
    Public ReadOnly [include2]() As Byte
    Public ReadOnly [legacy_biguint_to_str]() As Byte
    Public ReadOnly [loaded_method]() As Byte
    Public ReadOnly [lots_of_semi_colons]() As Byte
    Public ReadOnly [multiline_string]() As Byte
    Public ReadOnly [namespaces]() As Byte
    Public ReadOnly [negative_int]() As Byte
    Public ReadOnly [nested_class]() As Byte
    Public ReadOnly [nested_heap_access]() As Byte
    Public ReadOnly [nested_paragraph]() As Byte
    Public ReadOnly [nested_template]() As Byte
    Public ReadOnly [order_of_operators]() As Byte
    Public ReadOnly [pi_integral_0_1]() As Byte
    Public ReadOnly [primitive_template]() As Byte
    Public ReadOnly [reinterpret_cast]() As Byte
    Public ReadOnly [reinterpret_cast_heap]() As Byte
    Public ReadOnly [self_add]() As Byte
    Public ReadOnly [shift]() As Byte
    Public ReadOnly [str_unescape]() As Byte
    Public ReadOnly [struct_function_ref]() As Byte
    Public ReadOnly [struct_in_namespace]() As Byte
    Public ReadOnly [template]() As Byte
    Public ReadOnly [template_template_case1]() As Byte
    Public ReadOnly [template_with_different_length]() As Byte
    Public ReadOnly [template_wont_be_extended_twice]() As Byte
    Public ReadOnly [two_template_type_parameters]() As Byte
    Public ReadOnly [typedef]() As Byte
    Public ReadOnly [ufloat_operators]() As Byte
    Public ReadOnly [ufloat_std_out]() As Byte
    Public ReadOnly [var_delegate_template]() As Byte
    Public ReadOnly [while_0_to_1]() As Byte
    Public ReadOnly [while_1_to_100]() As Byte

    Sub New()
        [_1_to_100] = Convert.FromBase64String(strcat_hint(CUInt(220), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgaW50IHggPSAwOwogIGZvciAoaW50IGkgPSAwOyBpIDwgMTAwOyBpICs9IDEpIHsKICAgIHggPSB4ICsgaSArIDE7CiAgfQogIGIyc3R5bGU6OnN0ZF9vdXQoeCk7CiAgYjJzdHlsZTo6c3RkX291dCgiXG4iKTsKfQ=="))
        [__i] = Convert.FromBase64String(strcat_hint(CUInt(248), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQp2b2lkIG1haW4oKSB7DQogIGludCBpID0gMDsNCiAgKytpOw0KICBic3R5bGU6OnB1dGNoYXIoaSk7DQogICsraTsNCiAgYnN0eWxlOjpwdXRjaGFyKGkpOw0KICArK2k7DQogIGJzdHlsZTo6cHV0Y2hhcihpKTsNCiAgKytpOw0KICBic3R5bGU6OnB1dGNoYXIoaSk7DQp9DQo="))
        [__i__] = Convert.FromBase64String(strcat_hint(CUInt(856), _
        ""))
        [another_1_to_100] = Convert.FromBase64String(strcat_hint(CUInt(212), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgaW50IHggPSAwOwogIGZvciAoaW50IGkgPSAxOyBpIDw9IDEwMDsgaSArPSAxKSB7CiAgICB4ICs9IGk7CiAgfQogIGIyc3R5bGU6OnN0ZF9vdXQoeCk7CiAgYjJzdHlsZTo6c3RkX291dCgiXG4iKTsKfQ=="))
        [biguint] = Convert.FromBase64String(strcat_hint(CUInt(244), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYmlndWludCBpID0gNDI5NDk2NzI5Nkw7CiAgYmlndWludCBqID0gNDI5NDk2NzI5Nmw7CiAgYmlndWludCBrID0gMUw7CiAgYjJzdHlsZTo6c3RkX291dChpKTsKICBiMnN0eWxlOjpzdGRfb3V0KGopOwogIGIyc3R5bGU6OnN0ZF9vdXQoayk7Cn0="))
        [bool_and_bool] = Convert.FromBase64String(strcat_hint(CUInt(432), _
        ""))
        [calculate_pi_bbp] = Convert.FromBase64String(strcat_hint(CUInt(540), _
        ""))
        [case1] = Convert.FromBase64String(strcat_hint(CUInt(104), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYjJzdHlsZTo6c3RkX291dCgiSGVsbG8gV29ybGQiKTsKfQ=="))
        [case2] = Convert.FromBase64String(strcat_hint(CUInt(404), _
        ""))
        [class] = Convert.FromBase64String(strcat_hint(CUInt(424), _
        ""))
        [class_function_with_namespace] = Convert.FromBase64String(strcat_hint(CUInt(292), _
        ""))
        [class_in_namespace] = Convert.FromBase64String(strcat_hint(CUInt(528), _
        ""))
        [class_inheritance] = Convert.FromBase64String(strcat_hint(CUInt(920), _
        ""))
        [class_on_heap] = Convert.FromBase64String(strcat_hint(CUInt(732), _
        ""))
        [comments] = Convert.FromBase64String(strcat_hint(CUInt(440), _
        ""))
        [delegate] = Convert.FromBase64String(strcat_hint(CUInt(144), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQpkZWxlZ2F0ZSBpbnQgZnVuYyhpbnQsIGludCYpOw0KDQp2b2lkIG1haW4oKSB7DQogIGIyc3R5bGU6OnN0ZF9vdXQoImFiYyIpOw0KfQ=="))
        [delegate_template] = Convert.FromBase64String(strcat_hint(CUInt(560), _
        ""))
        [delegate2] = Convert.FromBase64String(strcat_hint(CUInt(308), _
        ""))
        [empty_struct] = Convert.FromBase64String(strcat_hint(CUInt(340), _
        ""))
        [errors_cycle_typedef] = Convert.FromBase64String(strcat_hint(CUInt(236), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQpuYW1lc3BhY2UgQ1lDTEVfVFlQRURFRiB7DQogIHR5cGVkZWYgQSBCOw0KICB0eXBlZGVmIEIgQzsNCiAgdHlwZWRlZiBDIEE7DQp9ICAvLyBuYW1lc3BhY2UgQ1lDTEVfVFlQRURFRg0KDQp2b2lkIG1haW4oKSB7DQogIENZQ0xFX1RZUEVERUY6OkEgYTsNCn0="))
        [errors_dollar_in_number] = Convert.FromBase64String(strcat_hint(CUInt(52), _
        "77u/DQp2b2lkIG1haW4oKSB7DQogIGZsb2F0IHggPSAxJDE7DQp9"))
        [errors_duplicate_template_type_parameters] = Convert.FromBase64String(strcat_hint(CUInt(44), _
        "77u/DQp0ZW1wbGF0ZTxULCBUPg0KY2xhc3MgQ3t9Ow=="))
        [errors_function_name_ends_with_dot] = Convert.FromBase64String(strcat_hint(CUInt(116), _
        "77u/DQpjbGFzcyBDew0KICBpbnQgeDsNCg0KICB2b2lkIHByaW50KCkge30NCn07DQoNCnZvaWQgbWFpbigpIHsNCiAgQyBjOw0KICBjLigpOw0KfQ=="))
        [errors_function_return_struct_type_mismatch] = Convert.FromBase64String(strcat_hint(CUInt(284), _
        ""))
        [errors_include_needs_wraps] = Convert.FromBase64String(strcat_hint(CUInt(52), _
        "77u/DQojaW5jbHVkZSBhYmMuaA0KDQp2b2lkIG1haW4oKSB7fQ=="))
        [errors_missing_ending_quota] = Convert.FromBase64String(strcat_hint(CUInt(92), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYjJzdHlsZTo6c3RkX291dCgiYWJjKTsKfQo="))
        [errors_reinterpret_cast_heap_with_index] = Convert.FromBase64String(strcat_hint(CUInt(252), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+CgpzdHJ1Y3QgUyB7CiAgaW50IHg7Cn07CgpzdHJ1Y3QgUzIgewogIGludCB4Owp9OwoKdm9pZCBtYWluKCkgewogIFMgc1sxXTsKICBTMl9fc3RydWN0X190eXBlX19pZF9fdHlwZSBzLlMyX19zdHJ1Y3RfX3R5cGVfX2lkWzFdOwogIHJlaW50ZXJwcmV0X2Nhc3Qoc1swXSwgUzIpOwp9Cg=="))
        [errors_reinterpret_cast_unknown_type] = Convert.FromBase64String(strcat_hint(CUInt(168), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIGYoaW50IHgpIHt9Cgp2b2lkIG1haW4oKSB7CiAgaW50IHg7CiAgcmVpbnRlcnByZXRfY2FzdCh4LCB0aGlzX2lzX2FuX3Vua25vd25fdHlwZSk7CiAgZih4KTsKfQo="))
        [errors_reinterpret_cast_unknown_variable] = Convert.FromBase64String(strcat_hint(CUInt(128), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgcmVpbnRlcnByZXRfY2FzdCh0aGlzX2lzX2FuX3Vua25vd25fdmFyaWFibGUsIGludCk7Cn0="))
        [errors_reinterpret_cast_without_type_id] = Convert.FromBase64String(strcat_hint(CUInt(576), _
        ""))
        [errors_template_without_type_parameter] = Convert.FromBase64String(strcat_hint(CUInt(132), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQp0ZW1wbGF0ZTw+DQpjbGFzcyBUaGlzVGVtcGxhdGVIYXNOb1R5cGVQYXJhbWV0ZXIge307DQoNCnZvaWQgbWFpbigpIHt9"))
        [errors_three_pluses] = Convert.FromBase64String(strcat_hint(CUInt(100), _
        "77u/DQojaW5jbHVkZSAgIDxiMnN0eWxlLmg+DQoNCnZvaWQgbWFpbigpIHsNCiAgaW50IGkgPSAwOw0KICBpICsrKzsNCn0NCg=="))
        [errors_undefined_value_clause] = Convert.FromBase64String(strcat_hint(CUInt(88), _
        "77u/DQp2b2lkIG1haW4oKSB7DQogIHRoaXNfaXNfYW5fdW5kZWZpbmVkX3ZhbHVlX2NsYXVzZSA9IDEwMDsNCn0="))
        [errors_value_clause_struct_type_mismatch] = Convert.FromBase64String(strcat_hint(CUInt(284), _
        ""))
        [errors_value_clause_struct_type_mismatch2] = Convert.FromBase64String(strcat_hint(CUInt(304), _
        ""))
        [for_loop] = Convert.FromBase64String(strcat_hint(CUInt(648), _
        ""))
        [function_ptr] = Convert.FromBase64String(strcat_hint(CUInt(456), _
        ""))
        [function_ref] = Convert.FromBase64String(strcat_hint(CUInt(968), _
        ""))
        [function_with_global_namespace] = Convert.FromBase64String(strcat_hint(CUInt(268), _
        ""))
        [heap] = Convert.FromBase64String(strcat_hint(CUInt(556), _
        ""))
        [heap_declaration] = Convert.FromBase64String(strcat_hint(CUInt(300), _
        ""))
        [heap_function_ref] = Convert.FromBase64String(strcat_hint(CUInt(340), _
        ""))
        [heap_ptr_to_int64] = Convert.FromBase64String(strcat_hint(CUInt(552), _
        ""))
        [i__] = Convert.FromBase64String(strcat_hint(CUInt(484), _
        ""))
        [i__2] = Convert.FromBase64String(strcat_hint(CUInt(248), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQp2b2lkIG1haW4oKSB7DQogIGludCBpID0gMDsNCiAgaSsrOw0KICBic3R5bGU6OnB1dGNoYXIoaSk7DQogIGkrKzsNCiAgYnN0eWxlOjpwdXRjaGFyKGkpOw0KICBpKys7DQogIGJzdHlsZTo6cHV0Y2hhcihpKTsNCiAgaSsrOw0KICBic3R5bGU6OnB1dGNoYXIoaSk7DQp9DQo="))
        [ifndef] = Convert.FromBase64String(strcat_hint(CUInt(212), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQojaW5jbHVkZSA8c3RkaW8uaD4NCiNpbmNsdWRlICJzdGRpby5oIg0KI2luY2x1ZGUgPGNzdGRpbz4NCiNpbmNsdWRlICJjc3RkaW8iDQoNCnZvaWQgbWFpbiAoKSB7DQogIGIyc3R5bGU6OnN0ZF9vdXQoImdvb2QiKTsNCn0NCg=="))
        [include] = Convert.FromBase64String(strcat_hint(CUInt(320), _
        ""))
        [include2] = Convert.FromBase64String(strcat_hint(CUInt(324), _
        ""))
        [legacy_biguint_to_str] = Convert.FromBase64String(strcat_hint(CUInt(488), _
        ""))
        [loaded_method] = Convert.FromBase64String(strcat_hint(CUInt(624), _
        ""))
        [lots_of_semi_colons] = Convert.FromBase64String(strcat_hint(CUInt(380), _
        ""))
        [multiline_string] = Convert.FromBase64String(strcat_hint(CUInt(112), _
        "CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYjJzdHlsZTo6c3RkX291dCgiYQogICAgYgogICAgYwogICAgZCIpOwp9Cg=="))
        [namespaces] = Convert.FromBase64String(strcat_hint(CUInt(720), _
        ""))
        [negative_int] = Convert.FromBase64String(strcat_hint(CUInt(152), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYjJzdHlsZTo6c3RkX291dCgtMSk7CiAgYjJzdHlsZTo6c3RkX291dCgtMik7CiAgYjJzdHlsZTo6c3RkX291dCgtMyk7Cn0="))
        [nested_class] = Convert.FromBase64String(strcat_hint(CUInt(388), _
        ""))
        [nested_heap_access] = Convert.FromBase64String(strcat_hint(CUInt(540), _
        ""))
        [nested_paragraph] = Convert.FromBase64String(strcat_hint(CUInt(348), _
        ""))
        [nested_template] = Convert.FromBase64String(strcat_hint(CUInt(436), _
        ""))
        [order_of_operators] = Convert.FromBase64String(strcat_hint(CUInt(152), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYjJzdHlsZTo6c3RkX291dCgxICsgMiAqIDMgKyA0IC0gNSAqIDYgKiA3ICogOCAqIDkgLyAxMCAqICgxMSArIDEyKSk7Cn0="))
        [pi_integral_0_1] = Convert.FromBase64String(strcat_hint(CUInt(296), _
        ""))
        [primitive_template] = Convert.FromBase64String(strcat_hint(CUInt(320), _
        ""))
        [reinterpret_cast] = Convert.FromBase64String(strcat_hint(CUInt(752), _
        ""))
        [reinterpret_cast_heap] = Convert.FromBase64String(strcat_hint(CUInt(512), _
        ""))
        [self_add] = Convert.FromBase64String(strcat_hint(CUInt(212), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgaW50IGkgPSAxMDA7CiAgaSArPSAxOwogIGIyc3R5bGU6OnN0ZF9vdXQoaSk7CiAgaSA9IGkrKzsKICBiMnN0eWxlOjpzdGRfb3V0KGkpOwogIGkgPSArK2k7CiAgYjJzdHlsZTo6c3RkX291dChpKTsKfQ=="))
        [shift] = Convert.FromBase64String(strcat_hint(CUInt(220), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgaW50IGkgPSAxMDA7CiAgaSA8PD0gMjsKICBiMnN0eWxlOjpzdGRfb3V0KGkpOwogIGludCBqID0gMDsKICBqID0gaSA+PiA1OwogIGIyc3R5bGU6OnN0ZF9vdXQoaSk7CiAgYjJzdHlsZTo6c3RkX291dChqKTsKfQ=="))
        [str_unescape] = Convert.FromBase64String(strcat_hint(CUInt(100), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgYjJzdHlsZTo6c3RkX291dCgiYWJjXHRkZWZcbiIpOwp9"))
        [struct_function_ref] = Convert.FromBase64String(strcat_hint(CUInt(272), _
        ""))
        [struct_in_namespace] = Convert.FromBase64String(strcat_hint(CUInt(312), _
        ""))
        [template] = Convert.FromBase64String(strcat_hint(CUInt(600), _
        ""))
        [template_template_case1] = Convert.FromBase64String(strcat_hint(CUInt(80), _
        "77u/DQp0ZW1wbGF0ZSA8VD4NCmNsYXNzIEMgew0KICBUIHg7DQogIHZvaWQgZihUIHkpIHt9DQp9Ow=="))
        [template_with_different_length] = Convert.FromBase64String(strcat_hint(CUInt(400), _
        ""))
        [template_wont_be_extended_twice] = Convert.FromBase64String(strcat_hint(CUInt(312), _
        ""))
        [two_template_type_parameters] = Convert.FromBase64String(strcat_hint(CUInt(104), _
        "77u/DQp0ZW1wbGF0ZTxULCBUMj4NCmNsYXNzIEMgew0KICBUIHg7DQogIFQyIHk7DQogIHZvaWQgcChUIHgsIFQyIHkpIHt9DQp9Ow=="))
        [typedef] = Convert.FromBase64String(strcat_hint(CUInt(228), _
        "77u/DQojaW5jbHVkZSA8YjJzdHlsZS5oPg0KDQpuYW1lc3BhY2UgYSB7DQogIHR5cGVkZWYgOjp2b2lkIHZvaWQ7DQogIHZvaWQgZigpIHsNCiAgICA6OmIyc3R5bGU6OnN0ZF9vdXQoImFiYyIpOw0KICB9DQp9ICAvLyBuYW1lc3BhY2UgYQ0KDQp2b2lkIG1haW4oKSB7DQogIGE6OmYoKTsNCn0NCg=="))
        [ufloat_operators] = Convert.FromBase64String(strcat_hint(CUInt(240), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgdWZsb2F0IGkgPSAyLjI7CiAgaSArPSAzLjA7CiAgaSAtPSAzLjA7CiAgaSAvPSAyLjA7CiAgYjJzdHlsZTo6c3RkX291dChpKTsKICBpID0gYjJzdHlsZTo6dWZsb2F0OjpmcmFjdGlvbigxLCAyKTsKICBiMnN0eWxlOjpzdGRfb3V0KGkpOwp9"))
        [ufloat_std_out] = Convert.FromBase64String(strcat_hint(CUInt(112), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgdWZsb2F0IGYgPSAxLjE7CiAgYjJzdHlsZTo6c3RkX291dChmKTsKfQ=="))
        [var_delegate_template] = Convert.FromBase64String(strcat_hint(CUInt(640), _
        ""))
        [while_0_to_1] = Convert.FromBase64String(strcat_hint(CUInt(236), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgdWZsb2F0IHggPSAwLjA7CiAgdWZsb2F0IGkgPSAwLjA7CiAgd2hpbGUgKGkgPCAxLjApIHsKICAgIGkgKz0gMC4wMTsKCXggKz0gaTsKICB9CiAgYjJzdHlsZTo6c3RkX291dCh4KTsKICBiMnN0eWxlOjpzdGRfb3V0KCJcbiIpOwp9Cg=="))
        [while_1_to_100] = Convert.FromBase64String(strcat_hint(CUInt(216), _
        "77u/CiNpbmNsdWRlIDxiMnN0eWxlLmg+Cgp2b2lkIG1haW4oKSB7CiAgaW50IHggPSAwOwogIGludCBpID0gMDsKICB3aGlsZSAoaSA8IDEwMCkgewogICAgaSArPSAxOwoJeCArPSBpOwogIH0KICBiMnN0eWxlOjpzdGRfb3V0KHgpOwogIGIyc3R5bGU6OnN0ZF9vdXQoIlxuIik7Cn0="))
    End Sub
End Module

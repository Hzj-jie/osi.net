    ...
    Public NotInheritable Class wrapper_register_in_multiple_threads_test
        ...
        Private Shared Sub run()
            ...
            assertion.is_true(wrapper.register(Function(ByVal v As var, ByVal i As test_class) As test_class
                                               ...
                                               End Function,
                                         index))
            assertion.is_true(wrapper(Of test_class).erase(index))
            assertion.is_true(wrapper.register(type,
                                         Function(ByVal v As var, ByVal i As test_class) As test_class
                                             Return New test_class(i)
                                         End Function,
                                         index))
            assertion.is_true(wrapper(Of test_class).erase(type, index))
        ...
        Private Shared Sub verify()
            ...
            assertion.is_true(wrapper.wrap(New var(), New test_class(), t))
            t.assert_wrap(thread_count)

            For i As UInt32 = 0 To thread_count - uint32_1
                assertion.is_true(wrapper.wrap(New var(strcat("--wrapper=", type_base, i)), New test_class(), t))
                t.assert_wrap(CInt(thread_count) + 1)
                assertion.is_true(wrapper(Of test_class).erase(strcat(type_base, i)))
            Next
            assertion.is_true(wrapper(Of test_class).erase())
        End Sub

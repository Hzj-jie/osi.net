...
Public Class configuration_test2
    ...
    Public Overrides Function run() As Boolean
        ' Test [build] section
        assert_true(config()("build")(build).to_bool())
        For i As UInt32 = 0 To config()("build").keys().size() - uint32_1
            Dim key As String = Nothing
            key = config()("build").keys()(i)
            assert_equal(config()("build")(key).to_bool(), strsame(build, key))
        Next
        ...
        v = variants.build()
        assert_equal(config()("section", v)("lucky", v).to_bool(), False)
        v = variants.with_time(1).build()
        assert_equal(config()("section", v)("lucky", v).to_bool(), True)
        v = variants.with_time(10).build()
        assert_equal(config()("section", v)("lucky", v), "unknown")

        v = variants.build()
        assert_equal(config()("section", v)("round", v).to_int32(), 1)
        v = variants.with_percentage(0.2).build()
        assert_equal(config()("section", v)("round", v).to_int32(), 0)
        ...
        v = variants.build()
        assert_equal(config()("section", v)("top50", v).to_bool(), True)
        assert_equal(config()("section", v)("bottom50", v).to_bool(), False)
        v = variants.with_user_name("aaa").build()
        assert_equal(config()("section", v)("top50", v).to_bool(), True)
        assert_equal(config()("section", v)("bottom50", v).to_bool(), False)
        v = variants.with_user_name("zzz").build()
        assert_equal(config()("section", v)("top50", v).to_bool(), False)
        assert_equal(config()("section", v)("bottom50", v).to_bool(), True)
        v = variants.with_user_name("mzzzzzzzzzzzzzzzzzzzz").build()
        assert_equal(config()("section", v)("top50", v).to_bool(), False)
        assert_equal(config()("section", v)("bottom50", v).to_bool(), False)

        v = variants.build()
        assert_equal(config()("section", v)("priority", v).to_int32(), 2)
        v = variants.with_reverse_host("com.joojle.www").build()
        assert_equal(config()("section", v)("priority", v).to_int32(), 1)
        v = variants.with_reverse_host("com.inlook.wap").build()
        assert_equal(config()("section", v)("priority", v).to_int32(), 3)
        v = variants.with_reverse_host("com.unknown.3g").build()
        assert_equal(config()("section", v)("priority", v).to_int32(), 99999)

        v = variants.build()
        assert_equal(config()("section", v)("search", v).to_bool(), True)
        ...
        assert_equal(config()("section", v)("search", v).to_bool(), False)

        v = variants.build()
        assert_equal(config()("section", v)("encrypted", v).to_bool(), False)
        v = variants.with_password("ThisIsAPassword").build()
        assert_equal(config()("section", v)("encrypted", v).to_bool(), True)

        v = variants.build()
        assert_equal(config()("section", v)("restricted", v).to_bool(), False)
        v = variants.with_processor_usage(100).build()
        assert_equal(config()("section", v)("restricted", v).to_bool(), True)
        ...
        assert_equal(config()("section", v)("top50", v).to_bool(), False)
        assert_equal(config()("section", v)("bottom50", v).to_bool(), False)
        assert_equal(config()("section", v)("priority", v).to_int32(), 99999999)
        assert_equal(config()("section", v)("search", v).to_bool(), False)
        assert_equal(config()("section", v)("encrypted", v).to_bool(), False)
        assert_equal(config()("section", v)("restricted", v).to_bool(), True)

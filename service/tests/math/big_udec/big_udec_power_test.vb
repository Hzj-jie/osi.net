...
Public NotInheritable Class big_udec_power_test
    ...
    Private Shared Sub calculate_pi_integral_0_01()
        Dim x As New big_udec()
        Dim dx As New big_udec(0.01)
        Dim s As New big_udec()
        Dim one As big_udec = Nothing
        one = big_udec.one()
        Dim two As New big_udec(2)
        Dim half As big_udec = big_udec.fraction(1, 2)
        While x < one
            s += ((one - x ^ two) ^ half) * dx
            x += dx
        End While

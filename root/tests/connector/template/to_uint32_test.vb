...
Public Class to_uint32_test
    ...
    Private Shared Function _uint32_to_uint32_case() As Boolean
        Dim x As New _uint32_to_uint32()
        assertion.equal(x.reverse(x(min_uint32)), min_uint32)
        assertion.equal(x.reverse(x(uint32_0)), uint32_0)
        assertion.equal(x.reverse(x(max_uint32)), max_uint32)
        Return True
    End Function

    Private Shared Function _int32_to_uint32_case() As Boolean
        Dim x As New _int32_to_uint32()
        assertion.equal(x.reverse(x(min_int32)), min_int32)
        assertion.equal(x.reverse(x(0)), 0)
        assertion.equal(x.reverse(x(max_int32)), max_int32)
        Return True
    End Function

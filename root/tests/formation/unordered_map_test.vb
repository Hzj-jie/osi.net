...
Public NotInheritable Class unordered_map_case(Of KEY_T, VALUE_T)
    ...
    Private Function random_select_key_from_u() As KEY_T
        ...
        Dim it As unordered_map(Of KEY_T, VALUE_T).iterator = u.begin()
        Dim c As UInt32 = rnd_uint(uint32_0, u.size())
        While c > uint32_0
            it += 1
            c -= uint32_1
        End While
    ...
    Private Sub insert_or_emplace(ByVal insert As Boolean)
        Dim n As KEY_T = rnd(Of KEY_T)()
        Dim v As VALUE_T = rnd(Of VALUE_T)()
    ...
    Private Sub [erase]()
        If rnd_bool() AndAlso Not m.empty() Then
            Dim k As KEY_T = random_select_key_from_u()
            assertion.is_true(m.erase(k))
            assertion.is_true(u.erase(k))
        Else
            Dim k As KEY_T = rnd(Of KEY_T)()
    ...
    Private Sub clone()
        ...
        Dim it As unordered_map(Of KEY_T, VALUE_T).iterator = u2.begin()
        While it <> u2.end()

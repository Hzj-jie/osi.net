...
#End If
    ...
    Public NotInheritable Class first_hasher(Of _HASHER As _to_uint32(Of FT))
        Inherits _to_uint32(Of first_const_pair(Of FT, ST))

        Private Shared ReadOnly hasher As _HASHER = alloc(Of _HASHER)()

        Public Overrides Function at(ByRef k As first_const_pair(Of FT, ST)) As UInt32
            assert(Not k Is Nothing)
            Return hasher(k.first)
        End Function

        Public Overrides Function reverse(ByVal i As UInt32) As first_const_pair(Of FT, ST)
            assert(False)
            Return Nothing
        End Function
    End Class
    ...
    Public NotInheritable Class first_equaler(Of _EQUALER As _equaler(Of FT))
        Inherits _equaler(Of first_const_pair(Of FT, ST))

        Private Shared ReadOnly equaler As _EQUALER = alloc(Of _EQUALER)()

        Public Overrides Function at(ByRef i As first_const_pair(Of FT, ST),
                                     ByRef j As first_const_pair(Of FT, ST)) As Boolean
            Dim c As Int32
...
#End If
    ...
    Public Shared ReadOnly reverse As Func(Of first_const_pair(Of FT, ST), first_const_pair(Of ST, FT)) =
        Function(ByVal p As first_const_pair(Of FT, ST)) As first_const_pair(Of ST, FT)
            assert(Not p Is Nothing)
            Return first_const_pair.[of](p.second, p.first)
        End FUnction

    Public Shared ReadOnly emplace_reverse As Func(Of first_const_pair(Of FT, ST), first_const_pair(Of ST, FT)) =
        Function(ByVal p As first_const_pair(Of FT, ST)) As first_const_pair(Of ST, FT)
            assert(Not p Is Nothing)
            Return first_const_pair.emplace_of(p.second, p.first)
        End FUnction
...
#If Not IS_CONST AndAlso Not IS_FIRST_CONST Then
    ...
    Public Shared Function move(ByVal that As first_const_pair(Of FT, ST)) As first_const_pair(Of FT, ST)
        If that Is Nothing Then
            Return Nothing
        End If
        Dim r As New first_const_pair(Of FT, ST)()
        r.first = that.first

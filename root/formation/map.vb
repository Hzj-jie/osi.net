...
Public NotInheritable Class map(Of KEY_T, VALUE_T)
    ...
    Implements ICloneable, ICloneable(Of map(Of KEY_T, VALUE_T)),
               IEquatable(Of map(Of KEY_T, VALUE_T))
    ...
    Shared Sub New()
        ...
        json_serializer(Of map(Of KEY_T, VALUE_T)).container(Of first_const_pair(Of KEY_T, VALUE_T)).
            register_as_object()
        string_serializer(Of map(Of KEY_T, VALUE_T)).container(Of first_const_pair(Of KEY_T, VALUE_T)).register()
    End Sub
    ...
    Public Function exclude(ByVal that As map(Of KEY_T, VALUE_T)) As map(Of KEY_T, VALUE_T)
        ...
        Dim it As map(Of KEY_T, VALUE_T).iterator = begin()
        While it <> [end]()
            Dim tit As map(Of KEY_T, VALUE_T).iterator = that.find((+it).first)
